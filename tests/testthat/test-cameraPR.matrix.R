...
test_that("inter.gene.cor is valid", {
  ...
  expect_identical(
    err1,
    paste0("Length of `inter.gene.cor` must be 1 or the same length as index. ",
           "If the latter, names of `inter.gene.cor` should match names of ",
           "`index`.")
  )

  # Length of inter.gene.cor is correct, but names do not match names of index1
  err2 <- expect_error(
  ...
  suppressWarnings(
    expect_no_error(
      cameraPR.matrix(statistic, index2,
                      inter.gene.cor = structure(rep(0.01, length(index2)),
                                                 names = names(index2)))
    )
  )
})
